<?php
/**
 * Refactored Future Land Use Map Application Form Handler
 * Replace the existing POST handling in form_sp_insert_future_land_use_map_application.php
 * with this code block (lines 15-218 in original file)
 */

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        // Extract form data
        $formData = extractFutureLandUseMapFormData($_POST, $_FILES);
        ...
        if (!empty($errors)) {
            ...
            $fileData = processFutureLandUseMapFileUploads($_FILES);
            
            // Insert application
            $result = insertFutureLandUseMapApplication($conn, $formData, $fileData);
